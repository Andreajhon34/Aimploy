...
interface SlideTextButtonProps extends ComponentProps<typeof Button> {
...
export default function SlideTextButton({
  ...
  return (
    ...
    >
      <Button
        className={cn(
          "group relative inline-flex h-10 items-center [&_svg:not([class*='size-'])]:size-auto justify-center overflow-hidden p-0! rounded-lg px-8 font-medium text-md tracking-tighter transition-all duration-300 md:min-w-56",
          variantStyles,
          className,
        )}
        variant="plain"
      ...
      >
        <span className="relative inline-block transition-transform duration-300 ease-in-out group-hover:-translate-y-full">
          ...
          <span className="absolute top-full -left-15 w-max flex items-center gap-2 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
            <span className="font-medium text-base">{slideText}</span>
          </span>
        </span>
      </Button>
    </motion.div>
